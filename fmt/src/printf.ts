...
import { args, stdout } from "@gnome/process";
...
let inspect = function (value: unknown, options?: InspectOptions): string {
...
if (g.process) {
    ...
    noColorFlag = process.env.NO_COLOR === "1" ||
    ...
    inspect = function (value: unknown, options?: InspectOptions): string {
        return nodeInspect(value, options);
    };
} else if (g.Deno) {
    // deno-lint-ignore no-explicit-any
    const Deno = g.Deno as any;
    platform = Deno.build.os as string;
    inspect = function (value: unknown, options?: InspectOptions): string {
        return Deno.inspect(value, options);
    };

    noColorFlag = Deno.noColor;
}
 ...
 * | `i`   | argument as formatted by `Deno.inspect` or util.inspect                        |
 * | `I`   | argument as formatted by `Deno.inspect` or util.inspect in compact format      |
 *
 * ## Width and Precision
 *
 ...
 * @example
 * ```ts
 * import { printLn } from "@gnome/fmt/print";
 *
 ...
 * @example
 * ```ts
 * import { printLn } from "@gnome/fmt/printf";
 *
 * printLn("Hello", "World"); // Prints "Hello World\n"
 * ```
 */
