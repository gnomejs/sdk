...
export interface FFITypeToArgsType {
    [FFIType.char]: number;
...
export interface FFITypeToReturnsType {
    [FFIType.char]: number;
    [FFIType.int8_t]: number;
    [FFIType.i8]: number;
    [FFIType.uint8_t]: number;
 ...
 * goes to Fabrice Bellard and TinyCC maintainers for making this possible.
 */
export function CFunction(fn: FFIFunction & { ptr: Pointer }): CallableFunction & {
    /**
 ...
 * goes to Fabrice Bellard and TinyCC maintainers for making this possible.
 */
export function linkSymbols<Fns extends Record<string, FFIFunction>>(
    symbols: Fns,
): Library<Fns>;
