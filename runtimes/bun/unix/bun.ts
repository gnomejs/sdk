...
import type { GrEnt, LibcLibrary, PwEnt } from "./types";
import { ENAMETOOLONG, ERANGE, UnixError } from "./errno";
import { err, ok, Result } from "@gnome/optional";
import { toCString } from "./_utils";
...
export class BunLibc implements LibcLibrary {
    ...
    getHostName(): Result<string> {
        ...
        while (ret === ENAMETOOLONG) {
            ...
            if (ret === 0) {
                const last = buf.findIndex((v) => v === 0);
                const result = new TextDecoder().decode(buf.slice(0, last));
                return ok(result);
            }
    ...
    getGroupIds(): Result<Uint32Array> {
        ...
        const ret = this.#lib.symbols.getgroups(1024, buf);
        if (ret === -1) {
...
export function openLibc(): Result<LibcLibrary> {
    ...
    try {
        ...
        return err<LibcLibrary>(e as Error);
    }
}
